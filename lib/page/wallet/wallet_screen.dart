...
class WalletScreen extends StatelessWidget {
                            ...
                            child: GestureDetector(
                              onTap: () async {
                                if (_walletFormKey.currentState!.validate()) {
                                  // Get.back();
                                  if ((walletController.selectedRadioTile?.value ?? "").trim() == "") {
  ...
  startRazorpayPayment() {
    try {
      walletController.createOrderRazorPay(amount: double.parse(amountController.text).round(), extraData: null).then((value) {
        if (value != null) {
  ...
  void openCheckout({required amount, required orderId}) async {
    ...
    var options = {
      'key':  key,
      'amount': (num.tryParse(amount) ?? 0).round() * 100,
      'name': 'ZoCar',
